...
#S3 Bucket to capture Nexus ALB access logs
resource "aws_s3_bucket" "s3_bucket_nexus" {
...
resource "aws_s3_bucket_server_side_encryption_configuration" "s3bucket_encryption_nexus" {
  count = var.s3_bucket_exists == false ? 1 : 0
  bucket = aws_s3_bucket.s3_bucket_nexus[0].id
...
resource "aws_s3_bucket_policy" "s3bucket_policy_nexus" {
  count = var.s3_bucket_exists == false ? 1 : 0
  bucket = aws_s3_bucket.s3_bucket_nexus[0].id
  policy = file("bucket-policy.json")
  
  depends_on = [aws_s3_bucket_server_side_encryption_configuration.s3bucket_encryption_nexus]
}
...
resource "aws_lb" "nexus-application-loadbalancer" {
  ...
  depends_on = [aws_s3_bucket_policy.s3bucket_policy_nexus]
}
